...
class QTrader(object):
    def __init__(self, eta=1, transaction_cost=0.005, position=10000):
        ...
        # Get date
        self.stock_data = pd.merge(pd.read_csv('./tbill.csv', index_col='Date'),
                                   pd.read_csv('./^GSPC.csv', index_col='Date'), right_index=True,
        ...
        #                 if the return is higer than high average -> 1             #
    ...
    # Training for Q_holding
    def q_holdings(self, training_indexes, testing_indexes):

        # value iteration
        # Q-learning
        # iterating on State, Action, Reqard, and new State (State prime)
        ...
        # Learning QW is quite expensive
        # Dyna
        # learn Q
        # Update a mdoel Transitions and also rewards
        # Hallucinate
        ...
        for i in range(100):
            ...
            for date, row in factors.iterrows():
                ...
                if last_row is None or np.isnan(return_data.state):
                    ...
                    update = alpha * (
                        factors.loc[date, 'reward'] + discount * max(q[row.state].values()) - q[state][action])
    ...
    def graph_portfolios(self):
        ...
        plt.annotate("Buy and hold sharpe: {}\n QTrader sharpe: {}".format(self.sharpe(portfolios.buy_and_hold),
                                                                           self.sharpe(portfolios.qtrader)),
